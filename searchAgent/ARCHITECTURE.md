...
The agent starts a local SearXNG process for the `searxng` provider, auto-starts a local Google AI Mode browser-pool sidecar when Chromium and `puppeteer-core` are available, and then starts the bundled Ploinky AgentServer. The dedicated `assistos/search-agent` image is pinned by immutable digest in `manifest.json`. It bakes the pinned SearXNG source, Python environment, Chromium, and Puppeteer into `/opt/search-agent` on the maintained Node 24.20 Trixie runtime. The selected index passed native amd64 and arm64 SearXNG, browser, and Git transport gates in [publication run 33691418169](https://github.com/AssistOS-AI/container-image-builds/actions/runs/33691418169). The runtime install hook only validates that immutable contract and generates the writable `$HOME/searxng/settings.yml`; it never runs a system package manager or writes to `/opt`. `manifest.json` declares MCP readiness and does not define TCP readiness, Router HTTP routes, or provider API keys.
